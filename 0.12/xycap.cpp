...
void XYCap::GetFigData(int *pwidth, int* pheight, unsigned char** pfigdata)
{
	if (!pwidth || !pheight || !pfigdata)
	{
		std::cout << "NULL POINTER DETECTED!!" << std::endl;
		return;
	}
	*pwidth = this->desktopWidth;
	*pheight = this->desktopHeight;
	*pfigdata = this->figdata;
}